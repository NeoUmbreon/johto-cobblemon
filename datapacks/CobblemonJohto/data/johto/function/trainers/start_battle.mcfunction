...
$tbcs battle GEN_9_SINGLES @s vs $(tbcs_id) onwin {1:["tag @1 remove inTrainerBattle","tag @1 add defeated.$(trainer_id)","tag @1 remove dialogueLock","tag @e[type=cobblemon:npc,tag=trainer.$(trainer_id),limit=1] remove trainerBusy"],2:["tag @3 remove inTrainerBattle","tag @3 remove dialogueLock","tag @e[type=cobblemon:npc,tag=trainer.$(trainer_id),limit=1] remove trainerBusy"]}
